...
//  M permite aceeder a las variables de estado o dependencia
  useEffect(() => {
    Axios.get('http://localhost:3003/api/v1/product/list').then((res) =>{
  ...
  const add_product_db = () => {
    console.log(barcode_add+description_add+unit_cost_add + state_add)
    Axios.post('http://localhost:3003/api/v1/product/add',{
      barcode : barcode_add,
  ...
  const delete_product = (_id) => {
     Axios.delete('http://localhost:3003/api/v1/product/delete/'+ _id)
  }


  const update_product = (_id) => {
    console.log(_id)
    Axios.put('http://localhost:3003/api/v1/product/update',{
      _id: _id,
  ...
  return (
      ...
      <Container>
        <br /> <br />
        <h1 className="App-header">Administración Ventas</h1>
  ...
  <tbody>
    {
      products.map((value, key) =>
        <tr>
          ...
          <td>
            <Button variant="secondary"  onClick ={() => delete_product(value._id)
            }>Eliminar
            </Button>
          </td>
...
<Form>
  <Form.Group className="mb-3" controlId="formBasicBarcode">
    <Form.Label>Codigo de barras</Form.Label>
    <Form.Control  id='barcode_update' type="number" placeholder="aquí va el código producto" onChange = {(e)=> {
